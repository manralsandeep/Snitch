...
import { Link, useNavigate } from 'react-router-dom' // Changed from 'react-router' to 'react-router-dom' based on standard usage
...
const Cart = () => {
    ...
    const { handleGetCart, handleCreateCartOrder, handleVerifyCartOrder, handleRemoveCartItem, handleUpdateCartItem } = useCart()
    ...
    const handleUpdateQuantity = async (cartItemId, productId, currentQty, delta, stockLimit) => {
        if (delta === -1 && currentQty <= 1) return;
        if (delta === 1 && currentQty >= stockLimit) return;

        setQuantities(prev => ({
            ...prev,
            [productId]: currentQty + delta,
        }));

        try {
            const action = delta > 0 ? 'increment' : 'decrement';
            await handleUpdateCartItem({ cartItemId, action });
    ...
    const handleRemoveItem = async (cartItemId) => {
        const data = await handleRemoveCartItem({ cartItemId })
        await handleGetCart()
                        ...
                        <div className="w-full lg:w-[65%]">
                            ...
                            <div className="flex flex-col gap-6">
                                {cart.items.map(item => {
                                    ...
                                    const { product, variant: variantId, price, product: { _id }, _id: cartId } = item
                                            ...
                                            <div className="flex-1 flex flex-col justify-between">
                                                ...
                                                <div className="flex items-center justify-between flex-wrap gap-4">
                                                    ...
                                                    >
                                                        ...
                                                        <button
                                                            id={`qty-dec-${_id}`}
                                                            onClick={() => handleUpdateQuantity(cartItemId, _id, qty, -1, stock)}
                                                            disabled={qty <= 1}
                                                        ...
                                                        <button
                                                            ...
                                                            onClick={() => handleUpdateQuantity(cartItemId, _id, qty, 1, stock)}
                                                            disabled={qty >= stock}
                                                            className={`w-9 h-9 flex items-center justify-center text-sm font-light transition-colors ${qty >= stock ? 'opacity-30 cursor-not-allowed' : 'hover:opacity-60'}`}
                        ...
                        <div className="w-full lg:w-[35%] lg:sticky lg:top-28">
                            ...
                            >
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        handleCheckout(handleCreateCartOrder, handleVerifyCartOrder)
                                    }}
                                >
                                    Proceed to Checkout
                                </button>
